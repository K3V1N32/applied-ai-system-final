...
- Popularity is not shown to the model as a candidate attribute. Worth double-checking, but the UI does surface popularity post-hoc, so a user could develop a false sense that Gemini is popularity-aware when it isn't shown that field at all, a mismatch between what's explained and what's used.
...
| Could you do a quick proofread on README.md and model_card.md to make sure my grammer and spelling is ok? |